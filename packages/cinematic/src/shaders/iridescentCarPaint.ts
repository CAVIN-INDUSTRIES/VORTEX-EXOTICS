...
uniform float uIridescenceLUTBlend;
uniform vec2 uMouseInfluence;
uniform sampler2D uIridescenceLUT;
`;

const CHROME_UNIFORM_DECL = /* glsl */ `
uniform float uCinematicTime;
uniform float uFlakeDensity;
uniform float uIridescenceStrength;
uniform float uClearCoatIntensity;
...
function injectBodyPreamble(shader: { fragmentShader: string }): void {
  shader.fragmentShader = shader.fragmentShader.replace(
    "#include <common>",
    `#include <common>
${BODY_UNIFORM_DECL}
...
function injectChromePreamble(shader: { fragmentShader: string }): void {
  shader.fragmentShader = shader.fragmentShader.replace(
    "#include <common>",
    `#include <common>
${CHROME_UNIFORM_DECL}
...
function attachBodyUniforms(
  ...
  shader.uniforms.uIridescenceLUT = { value: lut };
  ...
  shader.uniforms.uIridescenceLUTBlend = { value: u.iridescenceLUTBlend };
}

function attachChromeUniforms(
  shader: { uniforms: Record<string, THREE.IUniform> },
  sharedTime: THREE.IUniform<number>,
  sharedMouse: THREE.IUniform<THREE.Vector2>,
  u: CinematicPaintUniforms
...
export function patchBodyPhysicalMaterial(
  ...
  phys.onBeforeCompile = (shader) => {
    injectBodyPreamble(shader);
    attachBodyUniforms(shader, sharedTime, sharedMouse, lut, uniforms);
...
export function patchChromePhysicalMaterial(
  ...
  phys.onBeforeCompile = (shader) => {
    injectChromePreamble(shader);
    attachChromeUniforms(shader, sharedTime, sharedMouse, uniforms);
